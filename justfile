...
publish-schemas namespace name version: schema
  ...
  if [ ! -e "./contracts//metadata.json" ]; then \
    echo "Please create metadata.json for module metadata"; exit; \
  fi

  tmp_dir="$(mktemp -d)"
  schema_out_dir="$tmp_dir///"
  contract_path="$(cargo tree -e normal -i  | cut -d '(' -f2 | cut -d ')' -f1)"
  ...
  git commit -m 'Add schemas for   '

  # Create a pull request using 'gh' CLI tool
  gh pr create --title 'Add schemas for   ' --body ""

## Exection commands ##

run-script script name +CHAINS:
  cargo run --bin  --package  --features="daemon-bin" -- --network-ids 
