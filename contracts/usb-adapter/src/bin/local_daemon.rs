...
use usb::{contract::interface::UsbInterface, UsbExecuteMsg, USB_ID};
...
fn main() -> anyhow::Result<()> {
    ...
    let adapter_namespace = Namespace::from_id(USB_ID)?;
    ...
    publisher.publish_adapter::<UsbInstantiateMsg, UsbInterface<Daemon>>(
        UsbInstantiateMsg {},
    )?;
    ...
    let adapter = account.install_adapter::<UsbInterface<_>>(&[])?;

    // // Import adapter's endpoint function traits for easy interactions.
    use usb::msg::UsbQueryMsgFns;
    ...
    adapter.execute(
        &AdapterRequestMsg {
            ...
            request: UsbExecuteMsg::SetStatus {
